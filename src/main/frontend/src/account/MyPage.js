...
export default function MyPage() {
  ...
  const monthlyData = useMemo(
    () => (Array.isArray(mRows) ? mRows : []).map((d) => ({
  ...
  const onWithdrawClick = () => {
    if (providerName !== 'MYPLATE') {
      const ok = window.confirm('정말 탈퇴하시겠어요?');
      if (!ok) return;
      submitWithdraw(); // 소셜은 비번 없이 진행
      return;
    }
    setLeavePwd('');
    setShowLeave(true);
  ...
  const submitWithdraw = async () => {
    try {
      setLeaving(true);
      const body = providerName === 'MYPLATE' ? { password: leavePwd, reason: 'user_request' } : { reason: 'social' };
      ...
      const m = e?.response?.data?.message || e?.response?.data || e?.message || '탈퇴 처리 중 오류가 발생했습니다.';
      alert(m);
    } finally {
      setLeaving(false);
      setShowLeave(false);
  ...
  // 닉네임 편집
  const startEdit = () => { setNickname(me?.name ?? me?.username ?? ''); setError(''); setEditing(true); };
  const cancelEdit = () => { setEditing(false); setError(''); setNickname(me?.name ?? me?.username ?? ''); };
  const saveNick = async () => {
    const next = (nickname ?? '').trim();
    if (!NICK_RULE.test(next)) { setError('닉네임은 2~20자, 한글/영문/숫자/(_)(-)만 가능합니다.'); return; }
    if (next === (me?.name ?? me?.username ?? '')) { setEditing(false); return; }
    setSaving(true);
    setError('');
    try {
  ...
  return (
    <svg viewBox={`0 0 ${VB_W} ${VB_H}`} width="100%" height={height} preserveAspectRatio="none">
      {tickVals.map((v, i) => {
        ...
        return (
          <g key={i}>
            <line x1={P_LEFT} x2={VB_W - P_RIGHT} y1={y} y2={y} stroke={C_GRID} strokeDasharray="3 3" />
            <text x={P_LEFT - 8} y={y + 4} textAnchor="end" fontSize="12" fill={C_TEXT}>{v}</text>
          </g>
        );
      })}
      {[0,1,2,3,4,5].map((r) => {
      ...
      <path d={ratingPath} fill="none" stroke={C_LINE_RATING} strokeWidth="2" />
      {data.map((d, i) => {
        const y = yRating(d.rating);
        if (y == null) return null;
        return <circle key={i} cx={xCenter(i)} cy={y} r="3" fill={C_LINE_RATING} />;
      })}
      {data.map((d, i) => (
        <text key={i} x={xCenter(i)} y={VB_H - P_BOTTOM + 28} textAnchor="middle" fontSize="12" fill={C_TEXT}>
          {d.label}
        </text>
      ))}
      <line x1={P_LEFT} x2={VB_W - P_RIGHT} y1={P_TOP + plotH} y2={P_TOP + plotH} stroke={C_AXIS} />
      <line x1={P_LEFT} x2={P_LEFT} y1={P_TOP} y2={P_TOP + plotH} stroke={C_AXIS} />
      <line x1={VB_W - P_RIGHT} x2={VB_W - P_RIGHT} y1={P_TOP} y2={P_TOP + plotH} stroke={C_AXIS} />
      <g>
        <rect x={P_LEFT} y={8} width="10" height="10" fill={C_BAR_REVIEW} />
        <text x={P_LEFT + 16} y={18} fontSize="12">리뷰 수</text>
        <rect x={P_LEFT + 70} y={8} width="10" height="10" fill={C_BAR_BOOKMARK} />
        <text x={P_LEFT + 86} y={18} fontSize="12">즐겨찾기 수</text>
        <circle cx={P_LEFT + 170} cy={13} r="5" fill={C_LINE_RATING} />
