...
function useResetToken() {
    ...
    return (
        ...
        <p className="reset-sub">새 비밀번호를 입력해주세요.</p>

        <form onSubmit={onSubmit} className="reset-form">
            <label className="reset-field">
            <span className="reset-label">새 비밀번호</span>
            <input
                type="password"
                placeholder="6자 이상"
                value={pw}
            ...
            />
            {!pwValid && pw.length > 0 && (
                <span className="reset-help">비밀번호는 6자 이상이어야 합니다.</span>
            )}
            </label>

            <label className="reset-field">
            <span className="reset-label">비밀번호 확인</span>
            <input
                type="password"
                placeholder="다시 입력"
                value={pw2}
            ...
            />
            {!pwSame && pw2.length > 0 && (
                <span className="reset-help">비밀번호가 일치하지 않습니다.</span>
            )}
            </label>

            <button className="reset-btnPrimary" disabled={disabled}>
            {loading ? '변경 중…' : '비밀번호 변경'}
            </button>
        </form>

        {msg && <div className="reset-hint">{msg}</div>}

        <div className="reset-foot">
            문제가 있나요? <Link to="/forgot" className="reset-link">다시 링크 받기</Link>
        </div>
        </AuthLayout>
    );
    }