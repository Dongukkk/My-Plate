...
export default function Register() {
  ...
  return (
    ...
    <AuthLayout title="My Plate">
      <h2 className="register-title">새 사용자 등록</h2>
      <p className="register-sub">맛집 추천을 개인화하려면 계정을 만들어주세요.</p>

      <form onSubmit={onSubmit} className="register-form">
        <label className="register-field">
          <span className="register-label">사용자 이름</span>
          <input
            type="text"
            placeholder="홍길동"
            value={name}
        ...
        <label className="register-field">
          <span className="register-label">이메일</span>
          <input
            type="email"
          ...
          {!emailValid && email.length > 0 && (
            <span className="register-help">올바른 이메일 형식을 입력하세요.</span>
          )}
        </label>

        <label className="register-field">
          <span className="register-label">비밀번호</span>
          <input
            type="password"
            placeholder="6자 이상"
            value={pw}
          ...
          />
          {!pwValid && pw.length > 0 && (
            <span className="register-help">비밀번호는 6자 이상이어야 합니다.</span>
          )}
        </label>

        <label className="register-field">
          <span className="register-label">비밀번호 확인</span>
          <input
            type="password"
          ...
          />
          {!pwSame && pw2.length > 0 && (
            <span className="register-help">비밀번호가 일치하지 않습니다.</span>
          )}
        </label>
        ...
        <button className="register-btnPrimary" disabled={disabled}>
          {loading ? '가입 중…' : '회원가입'}
        </button>
      </form>

      {msg && <div className="register-hint">{msg}</div>}

      <div className="register-foot">
        이미 계정이 있으신가요? <Link to="/login" className="register-link">로그인</Link>
      </div>
    </AuthLayout>
  );
}